...
my $names = {
  ...
  TA_JS        => 'Template::Alloy - JavaScript code based',
  TA_JSS       => 'Template::Alloy - JavaScript code based using process_simple',
...
mkpath($dir2);
mkpath($dir3);
END {rmtree $dir; rmtree $dir2; rmtree $dir3};
my @dirs = ($dir);
...
my $tests = {
    ...
    TA_H_JS_str => sub {
        my $t = Template::Alloy->new(    type => 'scalarref', source => \$content_ht, case_sensitve=>1, cache => 1, COMPILE_JS => 1);
        $t->{'_documents'} = \%AlloyJS_DOCUMENTS;
        $t->param($stash_ht); $t->param($form); my $out = $t->output;
    },
...
#        return $out;
    ...
    TA_JS_mem  => sub { my $out = ""; $tajs->process( 'foo.tt', $form, \$out); $out },
    TA_JSS_mem => sub { my $out = ""; $tajss->process_simple( 'foo.tt', {%$stash_t, %$form}, \$out); $out },
