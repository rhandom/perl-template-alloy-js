...
my $tap = Template::Alloy->new(@config1, COMPILE_JS => 1);

use Template::Alloy;
my $tt1 = Template::Alloy->new(@config1);
...
my $tests = {                                                             #        #        #        #        #
    ...
    '09_mixed_med' => "".((" "x10)."[% one %] ")x100,                    #   35%  #  306%  #   97%  #  2156%  # 1017.8/s #
...
my $run = join("|", @run);
die "No matching tests for $run" if !grep {/$run/} sort keys %$tests;
...
foreach my $test_name (@run) {
    ...
    for (1..2) {
        if (file_Alloy() ne str_TT()) {
            ...
            debug file_Alloy(), str_TT();
            open(my $fh, ">", "bench.out.alloy.txt");
            print $fh file_Alloy();
            open($fh, ">", "bench.out.tt.txt");
            print $fh str_TT();
