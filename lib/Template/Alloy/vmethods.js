...
//    split    => \&vmethod_split,
  ...
  sprintf: function (s) {
    ...
    return (''+s).replace(/%(%|(?:(\d+)\$|)([-+#0 ]*)(?:|(?:([1-9]\d*)|(\*)(?:(\d+)\$|))(?:|\.(?:(\d+)|(\*)(?:|(\d+)\$))))([scbBoxXuidfegEG]))/g, function () {
      ...
      var flags = m[3];
      var type  = m[10];
      var width = parseInt(m[4] != null ? m[4] : m[5] ? a[++i] : m[6] ? a[m[6]] : 0);
      ...
      if (type == 's') { if (preci) val = (''+val).substr(0, preci) }
        ...
        if (/[xXbBo]/.test(type)) {
          var _p = flags.match(/#/);
          val = parseInt(val) >>> 0;
          if      (type == 'x') { if (_p) pre = '0x'; val = val.toString(16) }
          else if (type == 'X') { if (_p) pre = '0X'; val = val.toString(16).toUpperCase() }
          else if (type == 'b') { if (_p) pre = '0b'; val = val.toString(2) }
          else if (type == 'B') { if (_p) pre = '0B'; val = val.toString(2) }
          else if (type == 'o') { if (_p) pre = '0';  val = val.toString(8); if (pre && val == '0') pre = '' }
        } else {
          val = parseFloat(val);
          if (type == 'u') val = val >>> 0;
          else if (val < 0) { pre = '-'; val *= -1 }
          if (     type == 'e' || type == 'g') val = val.toExponent(preci == null ? 6 : preci);
          else if (type == 'E' || type == 'G') val = val.toExponent(preci == null ? 6 : preci).toUpperCase();
          else {
            val = val.toFixed(/[diu]/.test(type) ? 0 : preci == null ? 6 : preci);
            if (!pre) {
              if (flags.match(/\+/)) pre = '+';
              else if (flags.match(/\ /)) pre = ' ';
            }
          }
        }
        if (/0/.test(flags)) {
          if (preci == null || (preci < width && !/[dui]/.test(type))) preci = width;
          var diff = preci - val.length - (/[dui]/.test(type) ? 0 : pre.length);
      ...
      if (width && val.length < width) {
        ...
        val = flags.match(/-/) ? val + pad : pad + val;
      }
      return val;
    });
  },
