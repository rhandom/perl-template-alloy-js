...
dquote: function (s) { return (''+s).replace(/([\"\\])/g, '\\$1').replace(/\n/, '\\n') },
exp: function (s) { return Math.exp(s) },
fmt: null,
format: null,
hash: function (s) { return {'value': s} },
...
js: function (s) {
	if (!s) return '';
	return (''+s).replace(/\n/g,'\\n').replace(/\r/g,'\\r').replace(/^([\"\']+)/,function(m,a){return '\\'+a.split('').join('\\')}).replace(/([^\\])([\"\']+)/g,function(m,a,b){return a+'\\'+b.split('').join('\\')});
},
...
sprintf: function (s) {
	...
	return (''+s).replace(/%(%|(?:(\d+)\$|)((?:[-+#0 ]|'.)*)(?:|(?:([1-9]\d*)|(\*)(?:(\d+)\$|)))(?:|\.(?:(\d+)|(\*)(?:|(\d+)\$))))([scbBoxXuidfegEG])/g, function () {
		...
		if (type === 's') { val = preci ? (''+val).substr(0, preci) : val.toString() }
			...
			if      (type === 'x') { val = (val>>>0).toString(16); if (/#/.test(f)) pre = '0x' }
				...
				if      (type === 'e') val = val.toExponential(preci == null ? 6 : preci);
					...
					if (!pre) { if (/\+/.test(f)) { pre = '+' } else if (/\ /.test(f)) pre = ' ' }
				}
			}
			if (/0/.test(f)) {
		...
		if (width && val.length < width) {
			var pad = Array(1+width-val.length).join(/'/.test(f) ? f.charAt(f.indexOf("'")+1) : ' ');
...
squote: function (s) { return (''+s).replace(/([\'\\])/g, '\\$1') },
srand: function (s) { throw 'srand is not supported in javascript' },
...
grep: function (a, pat) {
	var b = [];
	if (typeof pat === 'function') { for (var i = 0; i < a.length; i++) { var n = pat(a[i], i); if (n && n !== '0') b.push(a[i]) } }
...
sort: function (a, field) {
	if (typeof field === 'function') return a.sort(field);
	var b = [];
...
$_hash.Text = $_hash.fmt; $_hash.List = $_hash.list; $_hash.Hash = $_hash.hash;
    return {item:$_item, list:$_list, hash:$_hash};
})();
