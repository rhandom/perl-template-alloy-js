var alloy = (function ($_call_native) {
var md5_hex, $_item, $_list, $_hash, $_vm, $_eval_recurse = 0;
function Alloy () { this.docs={}; this.blocks={} }

Alloy.prototype = {
call_native: function () {
  var r = $_call_native.apply(null, arguments);
...
process_s: function (str, argsa) {
  ...
  var undo = {};
  try {
    if (++$_eval_recurse > $_env.MAX_EVAL_RECURSE) this.throw('eval_recurse', "MAX_EVAL_RECURSE "+$_env.MAX_EVAL_RECURSE+" reached");
    ...
    for (var i in a) {
      ...
      undo[i] = $_env.hasOwnProperty(i) ? [$_env[i]] : 0;
      $_env[i] = a[i];
    }
  ...
  $_eval_recurse--;
  for (var i in undo) {
    if (!undo[i]) delete $_env[i];
    else $_env[i] = undo[i][0];
  }
  if (err) throw err;
  return out_ref[0];
},
...
load_vm: function () {
  ...
  $_item = $_vmethods.item;
  $_list = $_vmethods.list;
  $_hash = $_vmethods.hash;
},
...
iterator: function (items) {
  ...
  else if (typeof items == 'object') { if (!$_item) this.load_vm(); items = $_hash.pairs(items) }
  else items = [items];
  this.items = items;
  this.index = 0;
},
...
config: function (key, val) {
  ...
  var loc = /(ADD_LOCAL_PATH|CALL_CONTEXT|DUMP|VMETHOD_FUNCTIONS|STRICT)/;
  if (loc.test(key)) $_env[key] = val;
