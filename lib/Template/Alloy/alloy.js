...
process: function (path, out_ref, args) {
  ...
  if (! info) {
    if (alloy.blocks[path]) {
      info = alloy.blocks[path]._js;
    } else {
...
//      }
  ...
  for (var i = 0, I = files.length; i < I; i++) {
    ...
    if (typeof file != 'object' || file instanceof Array) {
      ...
      var old_val = alloy._process_dollar_template;
      var old_com = alloy._component;
      if (old_val) throw 'process - Recursion detected in '+dir+' \$template';
      alloy._process_dollar_template = 1;
      alloy._component = file;
...
//            }
      ...
      alloy._process_dollar_template = old_val;
      alloy._component = old_com;
    }
    out_ref[0]+=tmp_out[0];
    if (err) {
      throw 'process - unhandled ('+err+')';
