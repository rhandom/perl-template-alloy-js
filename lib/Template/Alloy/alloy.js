...
var md5_hex, $in = {}, $vmethods, $item, $list, $hash, $blocks = {}, $docs = {}, $eval_recurse = 0, $scopeVars, $scopeConfig, $scopeBlocks;
var Alloy = function () {
    var alloy = this;
    this._process = function (f, a, l, o) { return alloy.process_ex(f, a, l, o) };
    this._get = function (k) { return alloy.get(k) };
    this._set = function (k, v) { return alloy.set(k, v) };
};
...
register_template: function (path, info) { $docs[path] = info; return true },
insert: function (paths, out_ref) { out_ref[0] += $call_native('insert', paths) },
process: function (path, out_ref, top_level) {
  ...
  if (! info) {
    if ($blocks[path]) {
      ...
      this.call_native('load_template', path);
      info = $docs[path];
...
process_d: function (files, args, dir, out_ref) {
  if ($_env.NO_INCLUDES) this.throw('file', "NO_INCLUDES was set during a "+dir+" directive");
  for (var i = 0, I = args.length; i < I; i+=2) this.set(args[i], args[i+1]);
  for (var i = 0, I = files.length; i < I; i++) {
    ...
    if (typeof file !== 'object' || file instanceof Array) {
      ...
      if (old_val) this.throw('process', 'Recursion detected in '+dir+' \$template');
...
process_s: function (str, argsa) {
  ...
  if (!md5_hex) {
    var ref = this.call_native('load_jslib', 'md5');
    if (typeof ref === 'string') { eval(ref)  } else md5_hex = ref;
  ...
  try {
    ...
    this.call_native('load_template', path, str, a);
    var info = $docs[path];
    if (!info) this.throw('file', 'native_load handshake error - path '+path+' was not registered');
  ...
  $eval_recurse--;
  this.restoreConfig();
  if ($_env.CACHE_STR_REFS != null && $_env.CACHE_STR_REFS) delete $docs[path];
  if (err) throw err;
  return out_ref[0];
},
...
load_vm: function () {
  if ($vmethods) return $vmethods;
  var ref = (arguments.length == 1) ? arguments[0] : this.call_native('load_jslib', 'vmethods');
  if (typeof ref === 'string') { eval(ref); $vmethods = $_vmethods } else $vmethods = ref;
  if (!$vmethods) this.throw('undef', 'Failed to load vmethods js library');
  $item = $vmethods.item;
  $list = $vmethods.list;
  $hash = $vmethods.hash;
  return $_vmethods;
...
get: function (expr, ARGS, nctx) {
  if (typeof expr !== 'object') expr = [expr, 0]; //return expr;
  var i = 0;
  var name = expr[i++];
  var args = expr[i++];
  ...
  while (ref != null) {
    ...
    if (typeof ref === 'object') {
      ...
      if ($item[name]) {
        ...
        ref = name(ref); //try { ref = name(ref) } catch (e) { if (typeof e !== 'object' || ! e.type) this.throw('filter', e); throw e }; // if accessed directly causes 25% performance regression
      } else {
        var filter = $_env && $_env.FILTERS && $_env.FILTERS[name];
        if (!filter) filter = this.call_native('list_filters')[name];
        if (filter) {
          if (typeof filter === 'function') {
            ref = filter(ref); //try { ref = filter(ref) } catch (e) { if (typeof e !== 'object' || ! e.type) this.throw('filter', e); throw e };
          } else if (! (filter instanceof Array)) {
            this.throw('filter', 'invalid FILTER entry for '+name+' (not a CODE ref)');
            ...
            // try {
              ...
              if (filter[1]) { // it is a "dynamic filter" that will return a sub
                var _args = [];
                if (args) for (var j=0;j<args.length;j++) _args.push(typeof args[j] === 'function' ? args[j]() : args[j]);
                code = this.call_native('dynamic_filter', name, code, _args);
              }
              ref = code(ref);
...
if (typeof $_UNITTEST !== 'undefined') {
  Alloy.prototype._clear = function () { $in={}; $docs={}; $blocks={}; $eval_recurse=0; $scopeVars=null;  $scopeConfig=null; $scopeBlocks=null };
  Alloy.prototype._docs  = function () { return $docs };
}
