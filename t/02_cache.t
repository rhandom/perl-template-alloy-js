...
sub process_ok { # process the value and say if it was ok
    ...
    push @$conf, (INCLUDE_PATH => $test_dir, UNITTEST => 1);
...
sub pristine {
    ...
    if (my $ctx = $Template::Alloy::JS::js_context) {
        $ctx->eval(q{alloy._clear()});
    }
    flush_dir($test_dir);
    flush_dir($test_dir2);
...
sub test_cache {
    ...
    if ($not_ok) {
        ...
        print "#-------------------------\n";
        if (my $ctx = $Template::Alloy::JS::js_context) {
            my $docs = $ctx->eval(q{alloy._docs()}) || {};
            print "JS doc $_: $docs->{$_}->{'name'}\n" for sort keys %$docs;
            print "#-------------------------\n";
        }
    }
}
...
process_ok($name => 'BlueBAR', {blue => 'Blue', tt_config => [COMPILE_JS => 1]});
...
process_ok($name => 'BlueBAR', {blue => 'Blue', tt_config => [COMPILE_JS => 1, GLOBAL_CACHE => 1]});
...
process_ok($name => 'BlueBAR', {blue => 'Blue', tt_config => [COMPILE_JS => 1, COMPILE_EXT => '.ttc']});
...
process_ok($name => 'BlueBAR', {blue => 'Blue', tt_config => [COMPILE_JS => 1, COMPILE_DIR => $test_dir2]});
...
test_cache([$test_dir,  $name, 1],
           ...
           [$test_dir,  "$name$Template::Alloy::JS_COMPILE_EXT",  0],
           [$test_dir2, "$test_dir/$name$Template::Alloy::JS_COMPILE_EXT",  1],
           );
...
process_ok($name => 'BlueBAR', {blue => 'Blue', tt_config => [COMPILE_JS => 1, COMPILE_DIR => $test_dir2, COMPILE_EXT => '.ttc']});
...
process_ok($name => 'BlueBAR', {blue => 'Blue', tt_config => [COMPILE_JS => 1, COMPILE_DIR => $test_dir2, COMPILE_EXT => '.ttc', GLOBAL_CACHE => 1]});
...
process_ok($name => 'BlueBAR', {blue => 'Blue', tt_config => [GLOBAL_CACHE => 1, CACHE_STR_REFS => 0, COMPILE_JS => 1]});
...
process_ok($name => 'BlueBAR', {blue => 'Blue', tt_config => [GLOBAL_CACHE => 1, CACHE_STR_REFS => 0, COMPILE_JS => 1, FORCE_STR_REF_JS => 1]});
...
process_ok($name => 'BlueBAR', {blue => 'Blue', tt_config => [GLOBAL_CACHE => 1, COMPILE_JS => 1]});
...
process_ok($name => 'BlueBAR', {blue => 'Blue', tt_config => [COMPILE_DIR => $test_dir2, COMPILE_JS => 1]});
...
process_ok($name => 'BlueBAR', {blue => 'Blue', tt_config => [COMPILE_EXT => '.ttc', COMPILE_JS => 1]});
